CREATE TABLE IF NOT EXISTS game_data.participant_stats
(
    run_id UUID,
    matchid String CODEC (ZSTD(3)),
    participantid UInt8,
    puuid FixedString (78) CODEC (ZSTD(3)),
    teamid UInt8,
    summonerid String,
    summonerlevel UInt16,
    summonername String,
    riotidgamename String,
    riotidtagline String,
    profileicon UInt16,
    championid Nullable (Int32),
    championtransform UInt8,
    champlevel UInt8,
    champexperience UInt32,
    teamposition Enum8 (
        'UNKNOWN' = 0,
        'TOP' = 1,
        'JUNGLE' = 2,
        'MIDDLE' = 3,
        'BOTTOM' = 4,
        'UTILITY' = 5
    ),
    positionassignedbymatchmaking Nullable (String),
    selectedrolepreferences Nullable (String),
    win UInt8,
    gameendedinearlysurrender UInt8,
    gameendedinignbsurrender Nullable (UInt8),
    gameendedinsurrender UInt8,
    causedgameendfromignbsurrender Nullable (UInt8),
    teamearlysurrendered UInt8,
    teamignbsurrendered Nullable (UInt8),
    waspremadewithignbgameendcauser Nullable (UInt8),
    waspremadewithseveretransgressor Nullable (UInt8),
    wasseveretransgressor Nullable (UInt8),
    kills UInt8,
    deaths UInt8,
    assists UInt8,
    doublekills UInt8,
    triplekills UInt8,
    quadrakills UInt8,
    pentakills UInt8,
    killingsprees UInt8,
    largestkillingspree UInt8,
    largestmultikill UInt8,
    largestcriticalstrike UInt16,
    firstbloodkill UInt8,
    firstbloodassist UInt8,
    firsttowerkill UInt8,
    firsttowerassist UInt8,
    goldearned UInt32,
    goldspent UInt32,
    consumablespurchased UInt16,
    itemspurchased UInt16,
    item0 UInt16,
    item1 UInt16,
    item2 UInt16,
    item3 UInt16,
    item4 UInt16,
    item5 UInt16,
    item6 UInt16,
    totaldamagedealt UInt32,
    totaldamagedealttochampions UInt32,
    physicaldamagedealt UInt32,
    physicaldamagedealttochampions UInt32,
    magicdamagedealt UInt32,
    magicdamagedealttochampions UInt32,
    truedamagedealt UInt32,
    truedamagedealttochampions UInt32,
    damagedealttobuildings UInt32,
    damagedealttoturrets UInt32,
    damagedealttoobjectives UInt32,
    damagedealttoepicmonsters Nullable (UInt32),
    totaldamagetaken UInt32,
    physicaldamagetaken UInt32,
    magicdamagetaken UInt32,
    truedamagetaken UInt32,
    damageselfmitigated UInt32,
    totalheal UInt32,
    totalhealsonteammates UInt32,
    totalunitshealed UInt8,
    totaldamageshieldedonteammates UInt32,
    timeccingothers UInt16,
    totaltimeccdealt UInt16,
    totalminionskilled UInt16,
    neutralminionskilled UInt16,
    totalallyjungleminionskilled UInt16,
    totalenemyjungleminionskilled UInt16,
    baronkills UInt8,
    dragonkills UInt8,
    inhibitorkills UInt8,
    inhibitortakedowns UInt8,
    inhibitorslost UInt8,
    turretkills UInt8,
    turrettakedowns UInt8,
    turretslost UInt8,
    objectivesstolen UInt8,
    objectivesstolenassists UInt8,
    visionscore UInt16,
    wardsplaced UInt16,
    wardskilled UInt16,
    detectorwardsplaced UInt16,
    sightwardsboughtingame UInt16,
    visionwardsboughtingame UInt16,
    visionclearedpings UInt16,
    summoner1id UInt8,
    summoner2id UInt8,
    summoner1casts UInt16,
    summoner2casts UInt16,
    spell1casts UInt16,
    spell2casts UInt16,
    spell3casts UInt16,
    spell4casts UInt16,
    rolebounditem Nullable (UInt16),
    bountylevel Nullable (UInt8),
    playerbehaviorisheroincombat Nullable (UInt8),
    timeplayed UInt16,
    totaltimespentdead UInt16,
    longesttimespentliving UInt16,
    allinpings UInt16,
    assistmepings UInt16,
    basicpings UInt16,
    commandpings UInt16,
    dangerpings UInt16,
    enemymissingpings UInt16,
    enemyvisionpings UInt16,
    getbackpings UInt16,
    holdpings UInt16,
    needvisionpings UInt16,
    onmywaypings UInt16,
    pushpings UInt16,
    retreatpings Nullable (UInt16),
    unrealkills UInt8
)
ENGINE = MergeTree
ORDER BY (matchid, participantid, puuid);
