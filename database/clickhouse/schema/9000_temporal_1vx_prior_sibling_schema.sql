...
--
-- PRIOR 1 (sibling build): same champion, same role, sibling build.
--
-- Keyed by the *requesting* (championid, teamposition, build, phase); stat
-- values are the aggregates of the sibling build. Rows only exist for builds
-- that have a sibling pair:
--   ap                <-> ap_off_tank
...
--
-- Builds without a sibling do not appear in this table.
--
-- Utility isolation: sibling pairs are intra-family, so utility builds only
-- borrow from utility builds.

DROP TABLE IF EXISTS game_data_filtered.synergy_1vx_temporal_prior_sibling;

CREATE TABLE IF NOT EXISTS game_data_filtered.synergy_1vx_temporal_prior_sibling
(
