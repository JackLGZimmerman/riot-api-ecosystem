...
- Do not delegate small, obvious, single-file, tightly coupled, or urgent blocking work. The top-level agent may execute directly when delegation would add ceremony or risk.
- Prefer fewer sub-agents with sharper task packets over many agents with fuzzy mandates.

### Level 1: Orchestrator

- The top-level orchestration model owns strategy, integration, and accountability for the whole change.
...
- The orchestrator should continue meaningful non-overlapping work while sub-agents run instead of idly waiting.
...
- The review agent critiques the integrated result; it does not replace orchestrator accountability.
