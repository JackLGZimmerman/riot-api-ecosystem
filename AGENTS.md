...
- Default to top-level execution. Treat sub-agents as an explicit cost and risk tradeoff, not a habit.
...
- Executor sub-agents must not create nested sub-agent loops unless the top-level orchestrator explicitly authorized that shape.
...
- After significant or high-risk implementation, run a separate fresh-context review only when the added requests are justified; use top-level self-review for low-risk docs and small edits.
