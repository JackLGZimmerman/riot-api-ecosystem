...
from typing import Any, Callable, Dict, Iterable, List, Mapping, Sequence, Tuple, Optional, Set
...
def _get_in(obj: Any, path: Sequence[Tuple[str | int, str]]) -> Any:
    ...
    for key, label in path:
        try:
            ...
            raise SchemaError(f"Missing or invalid path: {'.'.join(l for _, l in path)} (failed at: {label})")
    return cur
...
def check_objectives(game: Mapping[str, Any]) -> List[str]:
    objs = _get_in(game, [("info","info"), ("teams","teams"), (0,"teams[0]"), ("objectives","objectives")])
...
def check_participants(game: Mapping[str, Any]) -> List[str]:
    p0 = _get_in(game, [("info","info"), ("participants","participants"), (0,"participants[0]")])
...
def check_perks(game: Mapping[str, Any]) -> List[str]:
    perks = _get_in(game, [("info","info"), ("participants","participants"), (0,"participants[0]"), ("perks","perks")])
...
class MatchSchemaCollector:
    ...
    strict: bool = True  # if False, missing sections produce empty lists instead of raising

    # registries allow easy extension / toggling of checks
    per_game: Mapping[str, CheckFn] = field(default_factory=lambda: {
    ...
    per_player: Mapping[str, CheckFn] = field(default_factory=lambda: {
