"""Inspect candidate farming specialist group semantics.

Run after a sweep to verify that retained PCA directions and group reads line
up with the metrics being tested:

    uv run python -m app.classification.embeddings.inspect_farming --kv 0.85 --t 0.70
"""
...
def _farming_spec() -> SpecialistSpec:
    for spec in SPECIALISTS:
        if spec.name == "farming":
            return spec
    raise ValueError("No active farming specialist found in SPECIALISTS")
...
def main() -> None:
    ...
    candidate = SpecialistSpec(
        "farming", features, args.t, args.kv, args.min_median
    )
    ...
    print(
        f"farming kv={args.kv:.2f} t={args.t:.2f} "
