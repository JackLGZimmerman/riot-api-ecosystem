"""Inspect candidate vision specialist group semantics.

Run after a sweep to verify that retained PCA directions and group reads line
up with the metrics being tested:

    uv run python -m app.classification.embeddings.inspect_vision --kv 0.85 --t 0.90
"""
...
def _vision_spec() -> SpecialistSpec:
    for spec in SPECIALISTS:
        if spec.name == "vision":
            return spec
    raise ValueError("No active vision specialist found in SPECIALISTS")
...
def main() -> None:
    ...
    candidate = SpecialistSpec(
        "vision", features, args.t, args.kv, args.min_median
    )
    ...
    print(
        f"vision kv={args.kv:.2f} t={args.t:.2f} "
