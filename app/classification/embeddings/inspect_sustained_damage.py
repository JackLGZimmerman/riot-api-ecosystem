"""Inspect candidate sustained-damage specialist group semantics.
...
def _sustained_damage_spec() -> SpecialistSpec:
    for spec in SPECIALISTS:
        if spec.name == "sustained_damage":
            return spec
    raise ValueError("No active sustained_damage specialist found in SPECIALISTS")
...
def main() -> None:
    ...
    candidate = SpecialistSpec(
        "sustained_damage", features, args.t, args.kv, args.min_median
    )
    ...
    print(
        f"sustained_damage kv={args.kv:.2f} t={args.t:.2f} "
    ...
    for gid, group in enumerate(sorted(kept, key=len, reverse=True), start=1):
        ...
        print(
            "  champs "
            + ", ".join(f"{k}:{v}" for k, v in champions.most_common(8))
        )
        print()
